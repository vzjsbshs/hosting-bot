...
async def main_menu(update, context):
    ...
    # Get user ID from either message or callback
    ...
    if ref:
        try:
            ...
            await context.bot.send_message(
                referrer_id,
                f"🎉 New referral! @{username or first_name} joined!\n+15 credits!"
            )
        except:
            pass
...
async def admin_pending(update, context):
    ...
    text = "📦 **PENDING ORDERS**\n\n"
    for o in orders:
        text += f"ID: `{o[0]}` | User: `{o[1]}` | Plan: {o[2]}\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_users, pattern='^users$'))
